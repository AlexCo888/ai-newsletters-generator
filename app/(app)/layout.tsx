  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      <div className="mx-auto grid w-full max-w-6xl gap-6 px-6 py-10 lg:grid-cols-[220px_1fr]">
        <aside className="hidden rounded-3xl border border-slate-800 bg-slate-900/50 p-6 lg:block">
          <div className="mb-8">
            <p className="text-xs uppercase tracking-wide text-slate-500">Signed in as</p>
            <p className="mt-2 text-sm font-medium text-white">{user?.fullName ?? user?.username ?? 'Subscriber'}</p>
            <p className="text-xs text-slate-400">{profile.email ?? user?.primaryEmailAddress?.emailAddress ?? ''}</p>
            <span
              className={`mt-4 inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${
                hasActiveSubscription ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/10 text-amber-300'
              }`}
            >
        ...
        <main className="flex flex-col gap-6">
          <header className="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-slate-900/60 px-5 py-4 shadow-lg shadow-slate-950/40 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-lg font-semibold text-white">AI Newsletters Studio</h1>
              <p className="text-sm text-slate-400">Craft, schedule, and deliver premium newsletters effortlessly.</p>
            </div>
